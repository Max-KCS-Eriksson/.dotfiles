...
# Generated by Powerlevel10k configuration wizard on 2023-05-19 at 11:57 CEST.
...
() {
  ...
  # Define two lines where certain elements will be hidden undre certain contidions.
  # See comment for below function `p10k-on-pre-prompt()` for more info.
    dir newline
    dir vcs newline os_icon vi_mode prompt_char
  )
  ...
  # Define two lines where certain elements will be hidden undre certain contidions.
  # See comment for below function `p10k-on-pre-prompt()` for more info.
    status command_execution_time background_jobs direnv asdf virtualenv anaconda pyenv goenv nodenv nvm nodeenv node_version go_version rust_version java_version rbenv rvm fvm luaenv jenv plenv perlbrew phpenv scalaenv haskell_stack kubecontext terraform aws aws_eb_env azure gcloud google_app_cred toolbox ranger nnn lf xplr vim_shell midnight_commander nix_shell todo timewarrior taskwarrior newline
    status command_execution_time background_jobs direnv asdf virtualenv anaconda pyenv goenv nodenv nvm nodeenv node_version go_version rust_version java_version rbenv rvm fvm luaenv jenv plenv perlbrew phpenv scalaenv haskell_stack kubecontext terraform aws aws_eb_env azure gcloud google_app_cred toolbox ranger nnn lf xplr vim_shell midnight_commander nix_shell todo timewarrior taskwarrior
  )
  ...
  # typeset -g POWERLEVEL9K_{LEFT,RIGHT}_{LEFT,RIGHT}_WHITESPACE=  # no surrounding whitespace, good if no powerbar
  typeset -g POWERLEVEL9K_LEFT_{LEFT,RIGHT}_WHITESPACE=  # no surrounding whitespace on left elements, good if left is no powerbar

  # Separator chars
  # Separator between same-color segments on the left.
  typeset -g POWERLEVEL9K_LEFT_SUBSEGMENT_SEPARATOR=' '  # separate with space
  # Separator between same-color segments on the right.
  typeset -g POWERLEVEL9K_RIGHT_SUBSEGMENT_SEPARATOR='\uE0B3'
  # Separator between different-color segments on the left.
  typeset -g POWERLEVEL9K_LEFT_SEGMENT_SEPARATOR=' '  # separate with space
  ...
  typeset -g POWERLEVEL9K_DIR_FOREGROUND=4  # 4, or 12 for blue
  ...
  typeset -g POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=4  # 4, or 12 for blue
  # Color of the anchor directory segments. Anchor segments are never shortened. The first
  # segment is always an anchor.
  typeset -g POWERLEVEL9K_DIR_ANCHOR_FOREGROUND=12  # 4, or 12 for blue - only matters if POWERLEVEL9K_DIR_ANCHOR_BOLD=false
  ...
  # Custom prefix.
  # typeset -g POWERLEVEL9K_DIR_PREFIX='%fin '  # Default
  # HACK: Workaround to get context without space before `dir`.
  if (( ! EUID )); then
    # Privileged user
    typeset -g POWERLEVEL9K_DIR_PREFIX='%F{red}%B%n@%m%f%b:'
  elif (( P9K_SSH )); then
    # Non-privileged user in SSH.
    typeset -g POWERLEVEL9K_DIR_PREFIX='%F{yellow}%B%n@%m%f%b:'
  else
    # Local non-privileged user.
    typeset -g POWERLEVEL9K_DIR_PREFIX='%F{green}%B%n@%m%f%b:'
  fi
  ...
  # Git status icons.
  # Change the value of these parameter to show a different icons.
  ...
  function my_git_formatter() {
    ...
    # NOTE: Leading `%b` on all formatting to escape potential bold formatting from previous formatting.
    if (( $1 )); then
      # Styling for up-to-date Git status.
      local       meta='%b%f'     # default foreground
      local      clean='%b%2F'    # green foreground
      local     staged='%b%12F'   # blue foreground
      local   modified='%b%3F'    # yellow foreground
      local  untracked='%b%1F'    # red foreground
      local conflicted='%b%1F%B'  # bold red foreground
    else
      # Styling for incomplete and stale Git status.
      local       meta='%b%f'     # default foreground
      local      clean='%b%f'     # default foreground
      local     staged='%b%f'     # default foreground
      local   modified='%b%f'     # default foreground
      local  untracked='%b%f'     # default foreground
      local conflicted='%b%f'     # default foreground
    fi
    ...
    # Leave undefined to turn off.
    local branch_prefix='['
    local branch_sufix=']'

    # Color depending on staged, unstaged/untracked, or not
    local has_conflicted='%1F%B'
    local has_unstaged='%1F'
    local has_untracked='%1F'
    local has_staged='%3F'

    local branch_color='%2F'
    (( VCS_STATUS_HAS_CONFLICTED )) && branch_color=${has_conflicted}
    (( VCS_STATUS_HAS_UNSTAGED )) && branch_color=${has_unstaged}
    (( VCS_STATUS_HAS_UNTRACKED )) && branch_color=${has_untracked}
    (( VCS_STATUS_HAS_STAGED )) && branch_color=${has_staged}
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      # Color branch name with `branch_color`
      branch_segment="${clean}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch_color}${branch//\%/%%}"  # Separate var for formatting
      # res+="${clean}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch_color}${branch//\%/%%}"  # Default
    fi

    # Format surrounding characters and branch
    res+="${branch_color}${branch_prefix}${branch_segment}${branch_sufix}"
    res+='%b%f'  # Escape bold and formatting
    ...
    # Color depending on ahead or behind
    local ahead='%b%3F'
    local behind='%b%1F%B'

    local commit_color=${clean}
    (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_PUSH_COMMITS_AHEAD )) && commit_color=${ahead}
    (( VCS_STATUS_COMMITS_BEHIND || VCS_STATUS_PUSH_COMMITS_BEHIND )) && commit_color=${behind}

    local commit_segment

    # ⇣42 if behind the remote.
    (( VCS_STATUS_COMMITS_BEHIND )) && commit_segment+=" ${behind}${POWERLEVEL9K_VCS_COMMITS_BEHIND_ICON}${VCS_STATUS_COMMITS_BEHIND}"
    # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && commit_segment+=" "
    (( VCS_STATUS_COMMITS_AHEAD  )) && commit_segment+="${ahead}${POWERLEVEL9K_VCS_COMMITS_AHEAD_ICON}${VCS_STATUS_COMMITS_AHEAD}"
    # ⇠42 if behind the push remote.
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && commit_segment+=" ${behind}${POWERLEVEL9K_VCS_PUSH_COMMITS_BEHIND_ICON}${VCS_STATUS_PUSH_COMMITS_BEHIND}"
    # ⇢42 if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
    (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && commit_segment+=" "
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && commit_segment+="${ahead}${POWERLEVEL9K_VCS_PUSH_COMMITS_AHEAD_ICON}${VCS_STATUS_PUSH_COMMITS_AHEAD}"

    # Only add segment if not empty
    if [[ $commit_segment ]]; then
        # Format surrounding characters and commits
        commit_prefix="${commit_color}${commit_prefix}"
        commit_sufix=" %8F${commit_sufix}"
        res+="${commit_prefix}${commit_segment}${commit_sufix}"
        res+='%b%f'  # Escape bold and formatting
    fi
    ...
    # *42 if have stashes.
    (( VCS_STATUS_STASHES        )) && res+=" ${clean}${POWERLEVEL9K_VCS_STASHES_ICON}${VCS_STATUS_STASHES}"
  ...
  typeset -g POWERLEVEL9K_ASDF_BACKGROUND=6
  ...
  typeset -g POWERLEVEL9K_CONTEXT_FOREGROUND=2  # 2, or 10 depending on blod is bright
  ...
  typeset -g POWERLEVEL9K_VIRTUALENV_{LEFT,RIGHT}_DELIMITER=  # Default
  # Custom icon.
  ...
  typeset -g POWERLEVEL9K_GOENV_BACKGROUND=6
  ...
  typeset -g POWERLEVEL9K_JAVA_VERSION_BACKGROUND=4
  ...
  typeset -g POWERLEVEL9K_PLENV_BACKGROUND=6
  ...
  typeset -g POWERLEVEL9K_PERLBREW_BACKGROUND=67
  ...
  typeset -g POWERLEVEL9K_PROXY_BACKGROUND=2
  # Custom icon.
  ...
  typeset -g POWERLEVEL9K_VI_COMMAND_MODE_STRING='%BWhere do you want to go%b'
  typeset -g POWERLEVEL9K_VI_MODE_NORMAL_FOREGROUND=8
  # Text and color for visual vi mode.
  typeset -g POWERLEVEL9K_VI_VISUAL_MODE_STRING='%BThe very best selection%b'
  typeset -g POWERLEVEL9K_VI_MODE_VISUAL_FOREGROUND=8
  # Text and color for overtype (a.k.a. overwrite and replace) vi mode.
  typeset -g POWERLEVEL9K_VI_OVERWRITE_MODE_STRING='%BEveryone makes mistakes%b'
  typeset -g POWERLEVEL9K_VI_MODE_OVERWRITE_FOREGROUND=8
  # Text and color for insert vi mode.
  typeset -g POWERLEVEL9K_VI_INSERT_MODE_STRING='%BYour command is my wish%b'
  ...
  typeset -g POWERLEVEL9K_TRANSIENT_PROMPT=off  # Use `p10k-on-pre-prompt()` and `p10k-on-post-prompt()` instead

  # Custom transient prompt
  function p10k-on-pre-prompt() {
    # Reset lines hidden in `p10k-on-post-prompt()`
    p10k display '1'=show '2'=show

    # Contitional prompt segments and line breaks
    # https://github.com/romkatv/powerlevel10k/issues/1276
    # The `p10k display` command allows you to hide and show segments or whole lines (among other things).
    # The "tricks" here are that `newline` isn't a segment that can be shown or hidden, and if an entire
    # line is hidden it won't show in the prompt as a blank line.
    # The solution therefore defines 2 lines for the right and left prompt. The second line is what is
    # desired when everything's on a single line. A first line is then defined that contains what should
    # be on the first line when we have it split across multiple lines. We then identify the items on the
    # single-line prompt (the second line in our config) that are also in the first line of the 2-line
    # prompt. When toggle between two states we are hiding the first prompt-line and showing everything
    # from the second-prompt line for the condition where we have everything on a single line, and switch
    # to showing both prompt lines but hiding the specific segments in the second-prompt line that are now
    # on the first prompt line when we want two lines in our prompt.
    emulate -L zsh -o extended_glob  # Allow for use of `#`, etc, for glob.
    if [[ -n ./(../)#(.git)(#qN/) ]]; then
      # Show line one, hide line two or elements in brackets from line two.
      p10k display '1'=show '2/left/(dir)'=hide '2/right'=hide
    else
      # Hide line one, show line two or elemints in brackets from line one.
      p10k display '1'=hide '2/left/(dir)'=show '2/right'=show
    fi
  }
  p10k-on-post-prompt () {
    # Hide lines if still in same dir.
    if [[ $prev_prompt_dir == $PWD ]]; then
      p10k display 'empty_line|1'=hide '2'=hide
    else
      p10k display '1'=show '2'=hide
    fi
